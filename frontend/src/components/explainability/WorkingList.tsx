  ...
  if (!working || working.length === 0) {
    return (
      <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-500 italic">
        {isTa ? 'கணக்கீட்டு விவரங்கள் குறிப்பிடப்படவில்லை.' : 'Detailed math steps are not specified for this estimate.'}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 uppercase tracking-wider">
        ...
        <span>{isTa ? 'மின்சல் கணக்கீட்டு முறை' : 'What MINCHAL Did (Calculation Math)'}</span>
      </div>
